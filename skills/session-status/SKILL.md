...
A `SessionStart` hook writes `$(date +%s)` to `~/.session-tracker/<session_id>/session-tracker`. The session ID is stable across compaction, so the timestamp survives context resets.
...
the shared awk that `SessionStart` deploys to `${SESSION_TRACKER_HOME:-$HOME/.session-tracker}/active-time.awk`:
...
Active time (`live.active_seconds`) is the headline; wall-clock (`live.elapsed_seconds`, "aberta há…") and idle time are shown as context only. `live.started_at` (epoch) formats to `desde HH:MM`. The reading grace after each Stop is configurable via `SESSION_IDLE_THRESHOLD_SECONDS` (default 120 = 2 minutes).
