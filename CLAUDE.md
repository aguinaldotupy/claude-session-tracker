...
Tests use the minimal harness in `tests/lib.sh` (`assert_eq`, `finish`). Each test file creates a `mktemp -d` and re-exports `HOME` into it, so tests never touch the real `~/.session-tracker/`. `tests/lib.sh` unsets `SESSION_IDLE_THRESHOLD_SECONDS` for determinism.
...
- **The opencode plugin adapts, it never reimplements.** Any change to time accounting, storage, or sync goes in the shell; `plugin.js` only ever translates an opencode hook into a `hooks/*.sh` invocation. If it starts computing something, that logic belongs in the shell instead.
