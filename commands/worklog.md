...
4. **Inline retroactive tagging.** `session-query` does not expose individual `session_id`s (it's a read-only aggregation CLI — writes, including tagging, stay out of its scope), and `/session-tracker:tag-session` only operates on `$HOME/.claude/session-env/history.jsonl`. So when `untagged.sessions > 0`, enumerate the untagged sessions to tag by reading `history.jsonl` directly (as before), applying the same date filter on `.start_ts`:
   ...
   - After all prompts, **re-run `session-query worklog`** so the freshly tagged sessions land in their issue's bucket. Sessions the user chose to skip remain in the Untagged bucket and are excluded from the posting plan.
...
5. **Introspect available MCP tools.** Scan the currently available tool names for patterns like `mcp__*Linear*`, `mcp__*atlassian*`, `mcp__*jira*`, `mcp__*Notion*`, `mcp__*toggl*`, `mcp__*clockify*`, or anything else that looks like a work tracker. Do NOT hardcode a list. Enumerate what you actually see and report it to the user.
6. For each grouped issue, propose a **posting plan** matched to one available MCP:
