...
1. If `~/.session-tracker/solidtime-sync.sh` does not exist, tell the user the session-tracker hooks haven't deployed the sync client yet (it ships on the next `SessionStart` — start a new session and retry) and stop.
...
4. Write the values into `~/.session-tracker/config.yml` with `st_config_set`, one key at a time (see the hint below). **Never regenerate `config.yml` wholesale** — it is the whole plugin's config file, so rewriting it by hand would drop any other section. `st_config_set` replaces exactly one key, creates the file `chmod 600` when missing, and quotes the value only when the parser would otherwise mangle it (Solidtime issues Laravel Sanctum tokens shaped `<id>|<random>`; the file is parsed, never sourced, so the `|` is just a character). Re-running this command is also how a user rotates a token or re-points sync at a different instance or organization (the client detects the change and discards its cached project/member ids automatically).
5. Verify with one real API call — `--check` makes a single `GET /users/me/memberships` against the instance instead of a normal sync run (which, with zero ended sessions queued, would make no HTTP call at all and could "verify" a wrong token or org by doing nothing) — and confirms the configured organization id is actually among the token's memberships, not just that the token is valid for *some* org:
   ```bash
   bash ~/.session-tracker/solidtime-sync.sh --check --verbose
   ```
...
```bash
ST="${SESSION_TRACKER_HOME:-$HOME/.session-tracker}"
. "$ST/db.sh"
st_config_set solidtime url    "$SOLIDTIME_URL"
st_config_set solidtime token  "$SOLIDTIME_TOKEN"
st_config_set solidtime org_id "$SOLIDTIME_ORG_ID"
bash "$ST/solidtime-sync.sh" --check --verbose
```

The result is a plain YAML file the user can also edit by hand:
